            ...
            if (shouldShowGhostInfo())
            {
                ...
                if (p == Arsonist.arsonist)
                    roleName += cs(Arsonist.color,
                        $" (剩余 {CachedPlayer.AllPlayers.Count(x => { return x.PlayerControl != Arsonist.arsonist && !x.IsDead && !x.Disconnected && !Arsonist.dousedPlayers.Any(y => y.PlayerId == x.PlayerId); })} )");
